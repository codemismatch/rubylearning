        ...
        const executeCode = async (runTests) => {
          ...
          try {
            ...
            const programLines = [
              ...
              "      file = $1",
              "      if File.exist?(file)",
              "        # Capture output of the script execution to emulate shell command output",
              "        original_buffer = $__exec_output_buffer__",
              "        $__exec_output_buffer__ = +\"\"",
              "        begin",
              "          eval(File.read(file))",
              "          output = $__exec_output_buffer__",
              "        rescue => e",
              "          output = \"ruby: \#{e.message}\\n\"",
              "        ensure",
              "          $__exec_output_buffer__ = original_buffer",
              "        end",
              "        output",
              "      else",
              "        \"ruby: No such file or directory -- \#{file}\\n\"",
              "      end",
              "    when /^echo\\s+(.+)$/",
              "      $1 + \"\\n\"",
              "    when /^mkdir\\s+(.+)$/",
